/**
 * Corrige monumentos con datos cruzados de Wikidata.
 * Para cada error confirmado:
 * 1. Busca en Wikidata el item correcto (nombre + municipio)
 * 2. Si encuentra match, actualiza con los datos correctos
 * 3. Si no, limpia los datos incorrectos (mejor nada que datos falsos)
 *
 * Uso: node fix_mismatches.cjs [startFrom]
 *   startFrom: índice 0-based desde donde continuar (default: 0)
 */
...
const HEADERS = { 'User-Agent': 'PatrimonioEuropeo/1.0 (data-fix)' };
const DELAY = 300;
...
(async () => {
  ...
  for (const row of r.rows) {
    ...
    if (!anyLocationMatch && !denomMatch && disambig.length > 2) {
      ...
      if (descMentionsDisambig && !descMentionsLocation) {
        mismatches.push(row);
      }
    }
  }

  console.log(`Errores confirmados: ${mismatches.length} | Empezando desde: ${START_FROM}\n`);

  let fixed = 0, cleaned = 0, errors = 0;
  ...
  for (let i = START_FROM; i < mismatches.length; i++) {
    ...
    try {
      ...
      if (foundQid && foundEntity) {
        ...
        console.log(`${progress} LIMPIADO  ID=${m.id} "${m.denominacion}" (${m.municipio || '?'}) | sin match`);
        cleaned++;
      }
    } catch (err) {
